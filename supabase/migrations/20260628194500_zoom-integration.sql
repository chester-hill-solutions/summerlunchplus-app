alter type public.app_permissions add value if not exists 'zoom_host.create';
alter type public.app_permissions add value if not exists 'zoom_host.read';
alter type public.app_permissions add value if not exists 'zoom_host.update';
alter type public.app_permissions add value if not exists 'zoom_host.delete';
alter type public.app_permissions add value if not exists 'class_zoom_meeting.create';
alter type public.app_permissions add value if not exists 'class_zoom_meeting.read';
alter type public.app_permissions add value if not exists 'class_zoom_meeting.update';
alter type public.app_permissions add value if not exists 'class_zoom_meeting.delete';
alter type public.app_permissions add value if not exists 'class_zoom_registrant.create';
alter type public.app_permissions add value if not exists 'class_zoom_registrant.read';
alter type public.app_permissions add value if not exists 'class_zoom_registrant.update';
alter type public.app_permissions add value if not exists 'class_zoom_registrant.delete';
alter type public.app_permissions add value if not exists 'class_zoom_participant_sync.create';
alter type public.app_permissions add value if not exists 'class_zoom_participant_sync.read';
alter type public.app_permissions add value if not exists 'class_zoom_participant_sync.update';
alter type public.app_permissions add value if not exists 'class_zoom_participant_sync.delete';
alter type public.app_permissions add value if not exists 'class_zoom_participant.create';
alter type public.app_permissions add value if not exists 'class_zoom_participant.read';
alter type public.app_permissions add value if not exists 'class_zoom_participant.update';
alter type public.app_permissions add value if not exists 'class_zoom_participant.delete';
alter type public.app_permissions add value if not exists 'zlr_click_event.create';
alter type public.app_permissions add value if not exists 'zlr_click_event.read';
alter type public.app_permissions add value if not exists 'zlr_click_event.update';
alter type public.app_permissions add value if not exists 'zlr_click_event.delete';
...
begin
  if not exists (
    select 1
    from pg_type
    where typname = 'zoom_meeting_status'
...
begin
  if not exists (
    select 1
    from pg_type
    where typname = 'zoom_sync_status'
      and typnamespace = 'public'::regnamespace
  ) then
...
create table if not exists public.zoom_host (
...
create table if not exists public.class_zoom_meeting (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists public.class_zoom_registrant (
  id uuid primary key default gen_random_uuid(),
  class_id uuid not null references public.class (id) on update cascade on delete cascade,
...
create table if not exists public.class_zoom_participant_sync (
  id uuid primary key default gen_random_uuid(),
  class_zoom_meeting_id uuid not null references public.class_zoom_meeting (id) on update cascade on delete cascade,
...
create table if not exists public.class_zoom_participant (
  id uuid primary key default gen_random_uuid(),
  class_zoom_meeting_id uuid not null references public.class_zoom_meeting (id) on update cascade on delete cascade,
  class_id uuid not null references public.class (id) on update cascade on delete cascade,
...
create table if not exists public.zlr_click_event (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists class_zoom_meeting_host_idx on public.class_zoom_meeting (zoom_host_id, start_time);
create index if not exists class_zoom_meeting_status_idx on public.class_zoom_meeting (status, start_time);
create index if not exists class_zoom_registrant_class_idx on public.class_zoom_registrant (class_id, profile_id);
create index if not exists class_zoom_participant_class_idx on public.class_zoom_participant (class_id, profile_id);
create index if not exists class_zoom_participant_email_idx on public.class_zoom_participant (lower(user_email));
create index if not exists class_zoom_participant_sync_meeting_idx on public.class_zoom_participant_sync (class_zoom_meeting_id, created_at desc);
create index if not exists zlr_click_event_registrant_idx on public.zlr_click_event (class_zoom_registrant_id, clicked_at desc);
...
drop policy if exists zoom_host_delete on public.zoom_host;
create policy zoom_host_delete on public.zoom_host
  for delete
  using (public.authorize('workshop.delete'));

drop policy if exists class_zoom_meeting_select on public.class_zoom_meeting;
create policy class_zoom_meeting_select on public.class_zoom_meeting
  for select
  using (public.authorize('workshop.read'));

drop policy if exists class_zoom_meeting_insert on public.class_zoom_meeting;
create policy class_zoom_meeting_insert on public.class_zoom_meeting
  for insert
  with check (public.authorize('workshop.create'));

drop policy if exists class_zoom_meeting_update on public.class_zoom_meeting;
create policy class_zoom_meeting_update on public.class_zoom_meeting
  for update
  using (public.authorize('workshop.update'))
  with check (public.authorize('workshop.update'));

drop policy if exists class_zoom_meeting_delete on public.class_zoom_meeting;
create policy class_zoom_meeting_delete on public.class_zoom_meeting
  for delete
  using (public.authorize('workshop.delete'));

drop policy if exists class_zoom_registrant_select on public.class_zoom_registrant;
create policy class_zoom_registrant_select on public.class_zoom_registrant
  for select
  using (public.authorize('workshop.read'));

drop policy if exists class_zoom_registrant_insert on public.class_zoom_registrant;
create policy class_zoom_registrant_insert on public.class_zoom_registrant
  for insert
  with check (public.authorize('workshop.update'));

drop policy if exists class_zoom_registrant_update on public.class_zoom_registrant;
create policy class_zoom_registrant_update on public.class_zoom_registrant
  for update
  using (public.authorize('workshop.update'))
  with check (public.authorize('workshop.update'));

drop policy if exists class_zoom_registrant_delete on public.class_zoom_registrant;
create policy class_zoom_registrant_delete on public.class_zoom_registrant
  for delete
  using (public.authorize('workshop.delete'));

drop policy if exists class_zoom_participant_sync_select on public.class_zoom_participant_sync;
create policy class_zoom_participant_sync_select on public.class_zoom_participant_sync
  for select
  using (public.authorize('workshop.read'));

drop policy if exists class_zoom_participant_sync_insert on public.class_zoom_participant_sync;
create policy class_zoom_participant_sync_insert on public.class_zoom_participant_sync
  for insert
  with check (public.authorize('workshop.update'));

drop policy if exists class_zoom_participant_sync_update on public.class_zoom_participant_sync;
create policy class_zoom_participant_sync_update on public.class_zoom_participant_sync
  for update
  using (public.authorize('workshop.update'))
  with check (public.authorize('workshop.update'));

drop policy if exists class_zoom_participant_sync_delete on public.class_zoom_participant_sync;
create policy class_zoom_participant_sync_delete on public.class_zoom_participant_sync
  for delete
  using (public.authorize('workshop.delete'));

drop policy if exists class_zoom_participant_select on public.class_zoom_participant;
create policy class_zoom_participant_select on public.class_zoom_participant
  for select
  using (public.authorize('class_attendance.read'));

drop policy if exists class_zoom_participant_insert on public.class_zoom_participant;
...
drop policy if exists class_zoom_participant_update on public.class_zoom_participant;
...
drop policy if exists class_zoom_participant_delete on public.class_zoom_participant;
...
drop policy if exists zlr_click_event_select on public.zlr_click_event;
create policy zlr_click_event_select on public.zlr_click_event
  for select
  using (public.authorize('class_attendance.read'));

drop policy if exists zlr_click_event_insert on public.zlr_click_event;
create policy zlr_click_event_insert on public.zlr_click_event
  for insert
  with check (public.authorize('class_attendance.update'));

drop policy if exists zlr_click_event_update on public.zlr_click_event;
create policy zlr_click_event_update on public.zlr_click_event
  for update
  using (public.authorize('class_attendance.update'))
  with check (public.authorize('class_attendance.update'));

drop policy if exists zlr_click_event_delete on public.zlr_click_event;
create policy zlr_click_event_delete on public.zlr_click_event
  for delete
  using (public.authorize('class_attendance.delete'));

drop policy if exists zoom_host_auth_admin on public.zoom_host;
create policy zoom_host_auth_admin on public.zoom_host
  for all to supabase_auth_admin
  using (true)
  with check (true);

drop policy if exists class_zoom_meeting_auth_admin on public.class_zoom_meeting;
create policy class_zoom_meeting_auth_admin on public.class_zoom_meeting
  for all to supabase_auth_admin
  using (true)
  with check (true);

drop policy if exists class_zoom_registrant_auth_admin on public.class_zoom_registrant;
create policy class_zoom_registrant_auth_admin on public.class_zoom_registrant
  for all to supabase_auth_admin
  using (true)
  with check (true);

drop policy if exists class_zoom_participant_sync_auth_admin on public.class_zoom_participant_sync;
create policy class_zoom_participant_sync_auth_admin on public.class_zoom_participant_sync
  for all to supabase_auth_admin
  using (true)
  with check (true);

drop policy if exists class_zoom_participant_auth_admin on public.class_zoom_participant;
create policy class_zoom_participant_auth_admin on public.class_zoom_participant
  for all to supabase_auth_admin
  using (true)
  with check (true);

drop policy if exists zlr_click_event_auth_admin on public.zlr_click_event;
