...
begin
  raise notice '[promote_user_after_submission] enter user % form %', new.user_id, new.form_id;

  if not public.should_auto_promote_onboarding() then
    raise notice '[promote_user_after_submission] skip: onboarding_mode=permission for user %', new.user_id;
    return new;
  end if;
