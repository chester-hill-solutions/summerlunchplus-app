...
CREATE OR REPLACE FUNCTION public.get_program_impact(p_semester_id uuid DEFAULT NULL::uuid, p_as_of timestamp with time zone DEFAULT now())
 RETURNS TABLE(semester_id uuid, semester_name text, family_key text, sent_card_count integer, blocked_sent_card_count integer, sent_card_value numeric, people integer, children integer, people_source text, children_source text, eligible_attendance_rows integer, completed_attendance_rows integer, participation_evidence_rows integer, newest_row_evidence_count integer, participation_classification text)
 LANGUAGE sql
 SECURITY DEFINER
