...
begin
  ...
  select coalesce(role, 'unassigned'::app_role) into current_role from public.user_roles where user_id = new.user_id;
  if current_role is distinct from 'unassigned' then
    raise log '[promote_user_after_submission] skip: role is % for user %', current_role, new.user_id;
    return new;
  end if;

  select coalesce(public.has_completed_required_forms(new.user_id), false) into has_completed;
  raise log '[promote_user_after_submission] eval user %, current_role %, has_completed %', new.user_id, current_role, has_completed;
