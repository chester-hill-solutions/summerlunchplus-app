...
- On initial user creation, the `on_auth_user_created_set_role` trigger injects the role from `raw_user_meta_data->>'role'` into `public.user_roles`. Ensure new roles are passed in via `signUp(options.data)` so the trigger can pick them up.
