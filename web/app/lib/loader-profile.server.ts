type LoaderProfileCheckpoint = {
  step: string
  durationMs: number
  extra?: Record<string, unknown>
}

const shouldLogLoaderProfile = () =>
  process.env.NODE_ENV !== 'production' ||
  process.env.VITE_ENABLE_ROUTER_INSTRUMENTATION === 'true'

export const createLoaderProfile = ({
  name,
  request,
}: {
  name: string
  ...
  const checkpoints: LoaderProfileCheckpoint[] = []
  ...
  const complete = (extra?: Record<string, unknown>) => {
    if (!shouldLogLoaderProfile()) return

    console.info('[loader-profile]', {
