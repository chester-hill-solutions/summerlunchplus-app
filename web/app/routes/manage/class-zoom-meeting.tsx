...
import type { Route } from './+types/class-zoom-meeting'
import TableDisplay from './table-display'
import { createTableAction } from './table-actions.server'
import { createTableLoader } from './table-loader'

const baseLoader = createTableLoader('class-zoom-meeting')
...
const classLabel = (value: MeetingRow['class_display']) => {
  if (typeof value === 'string') return value
  if (value && typeof value === 'object' && typeof value.label === 'string') return value.label
  return 'Workshop'
}

const classTimestamp = (value: MeetingRow['class_display']) => {
...
export async function loader(args: Route.LoaderArgs) {
  ...
  const rows = (base.rows ?? []) as MeetingRow[]
