...
export const TABLE_DEFINITIONS: Record<string, TableDefinition> = {
  ...
  'class-attendance-photo-upload-attempt': {
    ...
    select:
      'id, class_id, profile_id, class_attendance_id, uploaded_by, storage_bucket, storage_path, file_name, mime_type, byte_size, status, error_message, request_metadata, created_at, updated_at',
    columns: [
      'class_display',
      'profile_display',
      'status',
  ...
  'zoom-host': {
    label: 'Zoom Hosts',
    table: 'zoom_host',
    select: 'id, zoom_user_id, zoom_user_email, display_name, is_active, priority, notes, created_at, updated_at',
    columns: ['display_name', 'zoom_user_id', 'zoom_user_email', 'is_active', 'priority', 'notes', 'updated_at'],
    order: 'priority',
    orderAscending: true,
    editor: {
      primaryKey: ['id'],
      ...
      allowUpdate: true,
      fields: {
        zoom_user_id: { label: 'Zoom User ID', type: 'text', nullable: true },
        zoom_user_email: { label: 'Zoom User Email', type: 'text', nullable: true },
        display_name: { label: 'Display Name', type: 'text', nullable: true },
        is_active: { label: 'Active', type: 'boolean', required: true },
        priority: { label: 'Priority', type: 'number', required: true },
        notes: { label: 'Notes', type: 'text', nullable: true },
      },
    },
  },
  'class-zoom-meeting': {
    label: 'Class Zoom Meetings',
    table: 'class_zoom_meeting',
    select:
    ...
    lookupMappings: [
      ...
      {
        keyColumn: 'zoom_host_id',
        table: 'zoom_host',
        select: 'id, display_name, zoom_user_email, zoom_user_id',
        resultColumn: 'zoom_host_display',
    ...
    editor: {
      ...
      allowUpdate: true,
      fields: {
        class_id: { label: 'Class', type: 'foreign_key', required: true, foreignKeyTable: 'class' },
        zoom_host_id: { label: 'Zoom Host', type: 'foreign_key', required: true, foreignKeyTable: 'zoom_host' },
        zoom_meeting_id: { label: 'Zoom Meeting ID', type: 'text', nullable: true },
        zoom_meeting_uuid: { label: 'Zoom Meeting UUID', type: 'text', nullable: true },
        host_zoom_user_id: { label: 'Host Zoom User ID', type: 'text', nullable: true },
        host_zoom_user_email: { label: 'Host Zoom User Email', type: 'text', nullable: true },
        topic: { label: 'Topic', type: 'text', nullable: true },
        start_time: { label: 'Start Time', type: 'datetime', nullable: true },
        duration_minutes: { label: 'Duration Minutes', type: 'number', nullable: true },
        join_url: { label: 'Join URL', type: 'text', nullable: true },
        status: { label: 'Status', type: 'enum', required: true, enumValues: ['pending', 'created', 'failed', 'cancelled'] },
        error_message: { label: 'Error Message', type: 'text', nullable: true },
        last_synced_at: { label: 'Last Synced At', type: 'datetime', nullable: true },
      },
    },
  },
  'class-zoom-registrant': {
    label: 'Class Zoom Registrants',
    table: 'class_zoom_registrant',
    select:
      'id, class_id, profile_id, class_zoom_meeting_id, zoom_registrant_id, zoom_join_url, zlr_token_hash, zlr_expires_at, last_sent_at, created_at, updated_at',
    columns: [
      'class_display',
      'profile_display',
    ...
    editor: {
      ...
      fields: {
        class_id: { label: 'Class', type: 'foreign_key', required: true, foreignKeyTable: 'class' },
        profile_id: { label: 'Profile', type: 'foreign_key', required: true, foreignKeyTable: 'profile' },
        class_zoom_meeting_id: { label: 'Class Zoom Meeting', type: 'foreign_key', required: true, foreignKeyTable: 'class_zoom_meeting' },
        zoom_registrant_id: { label: 'Zoom Registrant ID', type: 'text', nullable: true },
        zoom_join_url: { label: 'Zoom Join URL', type: 'text', nullable: true },
        zlr_token_hash: { label: 'ZLR Token Hash', type: 'text', required: true },
        zlr_expires_at: { label: 'ZLR Expires At', type: 'datetime', nullable: true },
        last_sent_at: { label: 'Last Sent At', type: 'datetime', nullable: true },
      },
    },
  },
  'class-zoom-participant-sync': {
    label: 'Class Zoom Participant Sync',
    table: 'class_zoom_participant_sync',
    select: 'id, class_zoom_meeting_id, status, started_at, completed_at, error_message, payload, created_at',
    columns: ['class_zoom_meeting_display', 'status', 'started_at', 'completed_at', 'error_message', 'payload', 'created_at'],
    order: 'created_at',
    lookupMappings: [
      {
        keyColumn: 'class_zoom_meeting_id',
    ...
    editor: {
      ...
      fields: {
        class_zoom_meeting_id: { label: 'Class Zoom Meeting', type: 'foreign_key', required: true, foreignKeyTable: 'class_zoom_meeting' },
        status: { label: 'Status', type: 'enum', required: true, enumValues: ['pending', 'running', 'completed', 'failed'] },
        started_at: { label: 'Started At', type: 'datetime', nullable: true },
        completed_at: { label: 'Completed At', type: 'datetime', nullable: true },
        error_message: { label: 'Error Message', type: 'text', nullable: true },
        payload: { label: 'Payload JSON', type: 'json', required: true },
      },
    },
  },
  'class-zoom-participant': {
    label: 'Class Zoom Participants',
    ...
    editor: {
      ...
      fields: {
        class_zoom_meeting_id: { label: 'Class Zoom Meeting', type: 'foreign_key', required: true, foreignKeyTable: 'class_zoom_meeting' },
        class_id: { label: 'Class', type: 'foreign_key', required: true, foreignKeyTable: 'class' },
        profile_id: { label: 'Profile', type: 'foreign_key', nullable: true, foreignKeyTable: 'profile' },
        zoom_user_id: { label: 'Zoom User ID', type: 'text', nullable: true },
        user_name: { label: 'User Name', type: 'text', nullable: true },
        user_email: { label: 'User Email', type: 'text', nullable: true },
        join_time: { label: 'Join Time', type: 'datetime', nullable: true },
        leave_time: { label: 'Leave Time', type: 'datetime', nullable: true },
        duration_seconds: { label: 'Duration Seconds', type: 'number', nullable: true },
        camera_on: { label: 'Camera On', type: 'boolean', nullable: true },
        attentiveness_score: { label: 'Attentiveness Score', type: 'number', nullable: true },
        raw: { label: 'Raw JSON', type: 'json', required: true },
      },
    },
  },
  'zlr-click-event': {
    label: 'ZLR Click Events',
    table: 'zlr_click_event',
    select: 'id, class_zoom_registrant_id, profile_id, clicked_at, ip_address, user_agent, metadata',
    columns: ['class_zoom_registrant_display', 'profile_display', 'clicked_at', 'ip_address', 'user_agent', 'metadata'],
    order: 'clicked_at',
    lookupMappings: [
      {
        keyColumn: 'class_zoom_registrant_id',
        table: 'class_zoom_registrant',
        valueColumn: 'zoom_registrant_id',
        resultColumn: 'class_zoom_registrant_display',
      },
      {
        keyColumn: 'profile_id',
        table: 'profile',
    ...
    editor: {
      ...
      allowUpdate: true,
      fields: {
        class_zoom_registrant_id: { label: 'Class Zoom Registrant', type: 'foreign_key', required: true, foreignKeyTable: 'class_zoom_registrant' },
        profile_id: { label: 'Profile', type: 'foreign_key', nullable: true, foreignKeyTable: 'profile' },
        clicked_at: { label: 'Clicked At', type: 'datetime', required: true },
        ip_address: { label: 'IP Address', type: 'text', nullable: true },
        user_agent: { label: 'User Agent', type: 'text', nullable: true },
        metadata: { label: 'Metadata JSON', type: 'json', required: true },
      },
    },
  },
  form: {
  ...
  'ip-org-policy': {
    label: 'IP Org Policies',
    table: 'ip_org_policy',
    select: 'id, org_pattern, match_mode, policy_class, enabled, note, priority, created_at, updated_at',
    columns: [
      'org_pattern',
      'match_mode',
      'policy_class',
    ...
    editor: {
      ...
      allowUpdate: true,
      fields: {
        org_pattern: { label: 'Org pattern', type: 'text', required: true },
        match_mode: {
          label: 'Match mode',
          type: 'enum',
          required: true,
          enumValues: ['contains', 'exact', 'regex'],
        },
        policy_class: {
          label: 'Policy class',
          type: 'enum',
          required: true,
          enumValues: ['infra_proxy', 'consumer_isp', 'vpn_hosting_datacenter', 'trusted_enterprise', 'unknown'],
        },
        enabled: { label: 'Enabled', type: 'boolean', required: true },
        note: { label: 'Note', type: 'text', nullable: true },
        priority: { label: 'Priority', type: 'number', required: true },
      },
    },
  },
