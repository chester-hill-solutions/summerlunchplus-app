...
test('readiness check compares release_ready_at with now', async () => {
  expect(isReleaseReadyNow({ releaseReadyAt: '2026-07-10T16:00:00.000Z', now: Date.parse('2026-07-10T15:59:59.000Z') })).toBeFalsy()
  expect(isReleaseReadyNow({ releaseReadyAt: '2026-07-10T16:00:00.000Z', now: Date.parse('2026-07-10T16:00:00.000Z') })).toBeTruthy()
})
